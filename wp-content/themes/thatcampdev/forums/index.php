...
<?php get_header( 'thatcamp' ); ?>
	...
	<div id="content" class="clearfix" role="main">
			...
			<form action="" method="post" id="forums-search-form" class="dir-form">
				...
				<h3><?php _e( 'Forums Directory', 'thatcamp' ); ?><?php if ( is_user_logged_in() ) : ?> &nbsp;<a class="button show-hide-new" href="#new-topic" id="new-topic-button"><?php _e( 'New Topic', 'thatcamp' ); ?></a><?php endif; ?></h3>
			...
			<form action="" method="post" id="forums-directory-form" class="dir-form">
				...
				<div class="item-list-tabs" role="navigation">
					<ul>
						<li class="selected" id="forums-all"><a href="<?php echo trailingslashit( bp_get_root_domain() . '/' . bp_get_forums_root_slug() ); ?>"><?php printf( __( 'All Topics <span>%s</span>', 'thatcamp' ), bp_get_forum_topic_count() ); ?></a></li>

						<?php if ( is_user_logged_in() && bp_get_forum_topic_count_for_user( bp_loggedin_user_id() ) ) : ?>

							<li id="forums-personal"><a href="<?php echo trailingslashit( bp_loggedin_user_domain() . bp_get_forums_slug() . '/topics' ); ?>"><?php printf( __( 'My Topics <span>%s</span>', 'thatcamp' ), bp_get_forum_topic_count_for_user( bp_loggedin_user_id() ) ); ?></a></li>
				...
				<?php if ( is_user_logged_in() ) : ?>
					...
					<?php if ( bp_is_active( 'groups' ) && bp_has_groups( 'user_id=' . bp_loggedin_user_id() . '&type=alphabetical&max=100&per_page=100' ) ) : ?>
						...
						<form action="" method="post" id="forum-topic-form" class="standard-form">
							...
							<a name="post-new"></a>
							<h5><?php _e( 'Create New Topic:', 'thatcamp' ); ?></h5>

							<?php do_action( 'template_notices' ); ?>

							<label><?php _e( 'Title:', 'thatcamp' ); ?></label>
							<input type="text" name="topic_title" id="topic_title" value="" maxlength="100" />

							<label><?php _e( 'Content:', 'thatcamp' ); ?></label>
							<textarea name="topic_text" id="topic_text"></textarea>

							<label><?php _e( 'Tags (comma separated):', 'thatcamp' ); ?></label>
							<input type="text" name="topic_tags" id="topic_tags" value="" />

							<label><?php _e( 'Post In Group Forum:', 'thatcamp' ); ?></label>
							<select id="topic_group_id" name="topic_group_id">

								<option value=""><?php  _e( '----', 'thatcamp' ); ?></option>
							...
							<div class="submit">
								<input type="submit" name="submit_topic" id="submit" value="<?php _e( 'Post Topic', 'thatcamp' ); ?>" />
								<input type="button" name="submit_topic_cancel" id="submit_topic_cancel" value="<?php _e( 'Cancel', 'thatcamp' ); ?>" />
							</div>
					...
					<?php elseif ( bp_is_active( 'groups' ) ) : ?>
						...
						<div id="message" class="info">
							...
							<p><?php printf( __( "You are not a member of any groups so you don't have any group forums you can post in. To start posting, first find a group that matches the topic subject you'd like to start. If this group does not exist, why not <a href='%s'>create a new group</a>? Once you have joined or created the group you can post your topic in that group's forum.", 'thatcamp' ), site_url( bp_get_groups_root_slug() . '/create/' ) ); ?></p>
