...
function bizz_print_admin_scripts() {
	...
	if ( is_admin() && ('admin.php' == $pagenow) && ( $_GET['page'] == 'bizzthemes' || $_GET['page'] == 'bizz-design' ) ) {
		...
		wp_enqueue_script('jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/1.6.2/jquery.min.js'); // header
...
?>
	...
	// PrettyPhoto (lightbox)
	    jQuery(document).ready(function($){
	        jQuery("a[href*=.jpg],a[href*=.gif],a[href*=.png],a[href*=.swf],a[href*=.mov],a[rel^='prettyPhoto']").prettyPhoto({
