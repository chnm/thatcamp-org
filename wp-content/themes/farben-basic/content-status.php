	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		...
		<div class="entry-content description">
			<time class="date published updated" datetime="<?php echo esc_attr( get_the_date( 'Y-m-d' ) ) . 'T' . esc_attr( get_the_time( 'H:i' ) ) . 'Z'; ?>">
				<?php printf( __( 'Posted on %1$s at %2$s', 'farben-basic' ), get_the_date(), get_the_time() );	?>
			</time>

			<?php the_content( __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'farben-basic') ); ?>
	    </div><!-- .entry-content -->
