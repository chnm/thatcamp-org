...
class Obenland_WPMU_Network_Site_Users_Dropdown extends Obenland_Wp_Plugins_v300 {
	...
	public function network_site_users_after_list_table() {
		...
		<h3 id="add-user"><?php esc_html_e( 'Add User to This Site' ); ?></h3>
			<?php if ( current_user_can( 'create_users' ) AND apply_filters( 'show_network_site_users_add_new_form', true ) ) : ?>
		<p><?php esc_html_e( 'You may add from existing network users, or set up a new user to add to this site.' ); ?></p>
			<?php else : ?>
		<p><?php esc_html_e( 'You may add from existing network users to this site.' ); ?></p>
			<?php endif; ?>
		<h4 id="add-existing-user"><?php esc_html_e( 'Add Existing User' ); ?></h4>
...
function wpmunsud_instantiate() {
	...
	if ( is_network_admin() OR ( ! isset( $plugins[ plugin_basename( __FILE__ ) ] ) AND 'plugins.php' == $pagenow ) ) {
