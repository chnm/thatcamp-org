...
class Tests_Formatting_MakeClickable extends WP_UnitTestCase {
	...
	public function setUp() {
		parent::setUp();
		$this->set_option( array( 'nofollow' => true, 'strip_protocol' => false ) );
		add_filter( 'autohyperlink_urls_link_attributes', array( $this, 'remove_class_attribute_for_autohyperlink_urls' ) );
	}
	...
	protected function expect( $url = 'http://coffee2code.com', $text = '', $before = '', $after = '', $strip = true ) {
		$expected =  $before . '<a href="' . esc_attr( $url ) . '">' . $url . '</a>' . $after;

		return $expected;
	}

	protected function expect_email( $email = 'user@example.com', $text = '', $before = '', $after = '' ) {
		if ( empty( $text ) ) {
			$text = $email;
		}
...
*/
	...
	function test_no_links_within_links() {
		...
		foreach ( $in as $text ) {
			$this->assertEquals( $text, c2c_autohyperlink_link_urls( $text ) );
		}
	}
	...
	function test_line_break_in_existing_clickable_link() {
		...
		$this->assertEquals( $html, c2c_autohyperlink_link_urls( $html ) );
	}
