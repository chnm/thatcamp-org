...
abstract class c2c_AutoHyperlinkURLs_Plugin_041 {
	...
	protected function __construct( $version, $id_base, $author_prefix, $file, $plugin_options = array() ) {
		...
		if ( ! file_exists( $file ) ) {
			die( sprintf( __( 'Invalid file specified for C2C_Plugin: %s', 'auto-hyperlink-urls' ), $file ) );
		}
	...
	public function options_page_description( $localized_heading_text = '' ) {
		...
		if ( ! $this->disable_contextual_help ) {
			echo '<p class="see-help">' . __( 'See the "Help" link to the top-right of the page for more help.', 'auto-hyperlink-urls' ) . "</p>\n";
		}
	}
			...
			foreach ( $option_names as $opt ) {
				if ( !isset( $inputs[ $opt ] ) ) {
					if ( $this->config[ $opt ]['input'] == 'checkbox' ) {
						...
						$msg = sprintf( __( 'A value is required for: "%s"', 'auto-hyperlink-urls' ), $this->config[ $opt ]['label'] );
						...
						switch ( $this->config[ $opt ]['datatype'] ) {
							...
							case 'int':
								if ( ! empty( $val ) && ( ! is_numeric( $val ) || ( intval( $val ) != round( $val ) ) ) ) {
									$msg = sprintf( __( 'Expected integer value for: %s', 'auto-hyperlink-urls' ), $this->config[ $opt ]['label'] );
									$error = true;
									$val = '';
								}
								break;
	...
	public function verify_config() {
		...
		foreach ( $this->required_config as $config ) {
			if ( empty( $this->$config ) ) {
				die( sprintf( __( "The plugin configuration option '%s' must be supplied.", 'auto-hyperlink-urls' ), $config ) );
			}
		}
	...
	protected function load_textdomain() {
		load_plugin_textdomain( 'auto-hyperlink-urls' );
	}
	...
	public function contextual_help( $contextual_help, $screen_id, $screen = null ) {
		...
		$help = '<h3>' . __( 'More Plugin Help', 'auto-hyperlink-urls' ) . '</h3>';
		$help .= '<p class="more-help">';
		$help .= '<a title="' . esc_attr( sprintf( __( 'More information about %1$s %2$s', 'auto-hyperlink-urls' ), $this->name, $this->version ) ) .
			'" class="thickbox" href="' . $help_url . '">' . __( 'Click for more help on this plugin', 'auto-hyperlink-urls' ) . '</a>' .
			__( ' (especially check out the "Other Notes" tab, if present)', 'auto-hyperlink-urls' );
		$help .= ".</p>\n";
		return $help;
	}
...
HTML;
	...
	public function help_tabs_content( $screen ) {
		$screen->add_help_tab( array(
			...
			'title'   => __( 'More Help', 'auto-hyperlink-urls' ),
	...
	public function plugin_action_links( $action_links ) {
		$settings_link = '<a href="' . $this->settings_page . '.php?page='.$this->plugin_basename.'">' . __( 'Settings', 'auto-hyperlink-urls' ) . '</a>';
...
//				echo '<div class="hr">&nbsp;</div>' . "\n";
	...
	public function options_page() {
		...
		echo '<input type="submit" name="Submit" class="button-primary" value="' . esc_attr__( 'Save Changes', 'auto-hyperlink-urls' ) . '" />' . "\n";
		echo '<input type="submit" name="Reset" class="button" value="' . esc_attr__( 'Reset Settings', 'auto-hyperlink-urls' ) . '" />' . "\n";
		echo '</form>' . "\n";

		do_action( $this->get_hook( 'after_settings_form' ), $this );

		echo '<div id="c2c" class="wrap"><div>' . "\n";
		$c2c = '<a href="http://coffee2code.com" title="coffee2code.com">' . __( 'Scott Reilly, aka coffee2code', 'auto-hyperlink-urls' ) . '</a>';
